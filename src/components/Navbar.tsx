...
const ecosystemLinks = [
	...
	},
	{
		title: "MolVis",
		href: "/molvis",
		description: "The Visual Lens. Interactive 3D trajectory rendering.",
		...
		>
			<div className="container h-16 px-4 mx-auto flex justify-between items-center relative">
				...
				<nav className="hidden md:flex items-center gap-2 h-full">
						...
						<AnimatePresence>
							{dropdownOpen && (
								<motion.div
									...
									className="absolute top-full left-1/2 -translate-x-1/2 w-80 p-3 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.4)] z-50 overflow-hidden"
								>
									...
									<div className="relative grid gap-2">
										{ecosystemLinks.map((item, index) => (
											<motion.a
												...
												className={`flex flex-col gap-1 p-3 rounded-lg transition-all ${item.bg} group`}
											>
												<span
					...
					{isSubpage ? (
						<motion.a
							href="/molvis#features" /* Docs link should point to something meaningful for the landing page */
