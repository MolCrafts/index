...
/**
 * Static product entry — no entrance animation, no motion lift.
 * Hover only adjusts color/opacity (CSS), never position.
 */
function ProjectEntry({ item }: { item: EcosystemItem }) {
  return (
    ...
    >
      <div className="flex items-center gap-3.5">
        ...
        <div className="flex min-w-0 flex-col gap-0.5">
          ...
          >
            {item.title}
            {item.external && (
              <ArrowUpRight className="h-3.5 w-3.5 opacity-40 group-hover:opacity-90" />
            )}
          </span>
          <span className="flex flex-wrap items-center gap-2">
            <span className="text-[10px] font-semibold uppercase tracking-[0.28em] text-muted-foreground group-hover:text-foreground/70">
              {item.role}
            </span>
            {item.status && (
      ...
      <p className="text-sm font-light leading-relaxed text-muted-foreground group-hover:text-foreground/85 sm:text-[0.9375rem] sm:leading-[1.65]">
        {item.description}
      </p>
    </a>
  );
...
export const EcosystemArchitecture = () => {
  return (
    <section id="projects" className="relative overflow-hidden pb-24 pt-4 md:pb-32 md:pt-6">
      <div className={sectionContainer}>
        ...
        <div className="flex flex-col gap-16 md:gap-20">
          {ecosystemCategories.map((group) => (
            <div key={group.title} className="flex flex-col gap-8">
              ...
              <div className="grid grid-cols-1 gap-x-8 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:gap-x-12 xl:gap-y-12">
        ...
        <div className="mt-16 flex flex-wrap items-center gap-x-8 gap-y-4 pt-10 md:mt-20">
          <a
